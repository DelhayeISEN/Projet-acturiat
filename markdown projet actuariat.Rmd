...
glmfreq$aic
print("null.deviance")
glmfreq$null.deviance
print("deviance")
glmfreq$deviance
print("iter")
glmfreq$iter
predict1 <- predict(glmfreq, freMPL2.freq, type = 'response')
...
library(broom)
glmfreqtab <- tidy(glmfreq)
knitr::kable(glmfreqtab[,c("term","estimate")])
```

```{r, eval = FALSE}
recap1[recap1$difference<0.05,]
...
calcul_prime <- data.frame(
  ...
  prediction_moyenne_frequence = round(predict(glmfreq, freMPL2.posclaim, type = 'response'),2),
  prediction_moyenne_severite = round(predict(glmsev, freMPL2.posclaim, type = 'response'),2),
