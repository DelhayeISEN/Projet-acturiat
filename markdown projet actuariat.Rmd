...
glm2 <- glm(ClaimInd~DrivAge+HasKmLimit+offset(log(Exposure)), family=quasipoisson("log"), data=freMPL2)
summary(glm2)
#glm2$aic
#glm2$null.deviance
#glm2$deviance
...
```{r}
glm3 <- glm(ClaimAmount~DrivAge+VehAge+VehClass+VehBody+VehEnergy+offset(log(Exposure)), family=poisson("log"), data=freMPL2)
summary(glm3)
predict3 <- predict(glm3, freMPL2, type = 'response')
recap3 <- data.frame(ClaimAmount = freMPL2$ClaimAmount,
                     Predict_ClaimAmount = predict3,
                     difference = abs(freMPL2$ClaimAmount- predict3))
head(recap3,10)
```
