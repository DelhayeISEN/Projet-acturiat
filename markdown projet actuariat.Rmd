...
```{r}
for (i in 1:dim(freMPL2)[2]){
  # Type de véhicules
  if (colnames(freMPL2)[i]=="VehBody"){
      levels(freMPL2$VehBody) <- c(levels(freMPL2$VehBody), "autobus", "coupé",
  ...
  # Statut marital
  if (colnames(freMPL2)[i]=="VehBody"){
      levels(freMPL2$MariStat) <- c(levels(freMPL2$MariStat), "célibataire", "autre")
      freMPL2$MariStat[freMPL2$MariStat == "Alone"]<-"célibataire"
      freMPL2$MariStat[freMPL2$MariStat == "Other"]<-"autre"
      freMPL2$MariStat <- droplevels(freMPL2$MariStat)
  }
  # Utilisation du véhicule
  if (colnames(freMPL2)[i]=="VehUsage"){
       levels(freMPL2$VehUsage) <- c(levels(freMPL2$VehUsage), "privée", 
                                     "privée et trajet vers bureau", "professionnel", 
                                     "trajet professionnel" )
       freMPL2$VehUsage[freMPL2$VehUsage == "Private"]<-"privée"
       freMPL2$VehUsage[freMPL2$VehUsage == "Private+trip to office"]<-"privée et trajet vers bureau"
       freMPL2$VehUsage[freMPL2$VehUsage == "Professional"]<-"professionnel"
       freMPL2$VehUsage[freMPL2$VehUsage == "Professional run"]<-"trajet professionnel"
       freMPL2$VehUsage <- droplevels(freMPL2$VehUsage)
  }
  # Moteur du véhicule
  if (colnames(freMPL2)[i]=="VehUsage"){
      levels(freMPL2$VehEngine) <- c(levels(freMPL2$VehEngine), "injection directe surpuissante",
                                     "électrique", "injection surpuissante")
      freMPL2$VehEngine[freMPL2$VehEngine == "direct injection overpowered"]<-"injection directe surpuissante"
      freMPL2$VehEngine[freMPL2$VehEngine == "electric"]<-"électrique"
      freMPL2$VehEngine[freMPL2$VehEngine == "injection overpowered"]<-"injection surpuissante"
      freMPL2$VehEngine <- droplevels(freMPL2$VehEngine)
  }
  # Energie utilisée par le véhicule
  if (colnames(freMPL2)[i]=="VehEnergy"){
      levels(freMPL2$VehEnergy) <- c(levels(freMPL2$VehEnergy), "électrique", "essence")
      freMPL2$VehEnergy[freMPL2$VehEnergy == "regular"]<-"essence"
      freMPL2$VehEnergy[freMPL2$VehEnergy == "eletric"]<-"électrique"
      freMPL2$VehEnergy <- droplevels(freMPL2$VehEnergy)
  }
  # Garage
  if (colnames(freMPL2)[i]=="Garage"){
      levels(freMPL2$Garage) <- c(levels(freMPL2$Garage), "aucun", "garage indépendant", "concessionnaire")
      freMPL2$Garage[freMPL2$Garage == "None"]<-"aucun"
      freMPL2$Garage[freMPL2$Garage == "Private garage"]<-"garage indépendant"
      freMPL2$Garage[freMPL2$Garage == "Collective garage"]<-"concessionnaire"
      freMPL2$Garage <- droplevels(freMPL2$Garage)
  }
}
```
