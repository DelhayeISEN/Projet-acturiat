...
|    Ainsi, dans notre cas les distributions de Poisson ou binomiales négatives peuvent être utiliser pour représenter les fréquences des sinistres.
...
|    Elle prend en paramètres un modèle linéaire (ici, glmfreqfinal), une base de données sur laquelle effectuée ces prédictions (freMPL2) et le type	de prediction (response ou autre). Dans notre cas, nous avons choisit le type 'response' qui nous renvoie l'espérance moyenne de la prédiction (donc ici notre fréquence). L'autre type nous aurait renvoyer la somme des coefficients (donc non transformée par la fonction réciproque). 
...
De même, l'âge du véhicule n'influence pas la fréquence de sinistres ce qui explique probablement le fait que cette variable n'est pas été retenu pour établir notre modèle GLM.

### Sévérité des sinistres

#### Mise en place et exécution du modèle GLM

|    Après avoir déterminer la fréquence des sinistres, il nous faut maintenant calculer la sévérité de ces sinistres pour une police donnée.  
|    Pour celà, nous allons nous intéresser à la variable ClaimAmount qui correspond à un montant auquel peut prétendre l'assurer en cas d'accident. Si le montant est nulle, c'est que la personne n'a pas eu d'accident sinon c'est qu'il en a eu au moins un. 
|    Pour modéliser la sévérité des sinistres, la loi la plus utilisée est la loi de probabilité Gamma. C'est donc cette loi que nous allons utilisé pour faire notre prédiction. Le résultat de la prédiction de notre modèle GLM correspondra à une valeur numérique traduisant la sévérité de l'accident : Plus la valeur sera élévée, plus l'accident sera grave.
```{r}
freMPL2 <- freMPL2save
```
...
|    Pour le type de loi, nous utilisons donc la famille Gamma et la fonction de lien associé log qui est le logarithme népérien. 
