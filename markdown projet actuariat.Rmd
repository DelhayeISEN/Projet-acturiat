...
La fonction fviz_contrib() [package factoextra] peut être utilisée pour créer un bar plot de la contribution des variables. Si vos données contiennent de nombreuses variables, vous pouvez décider de ne montrer que les principales variables contributives.

```{r}
fviz_contrib(freMPL2.pca, choice = "var", axes = 1)
fviz_contrib(freMPL2.pca, choice = "var", axes = 2)
fviz_contrib(freMPL2.pca, choice = "var", axes = 3)
```

On peut également visualiser la contribution totale aux deux premiers axes. 

```{r}
fviz_contrib(freMPL2.pca, choice = "var", axes = 1:2)
```
