...
nettoyage_dataframe <- function(dt){
  ...
  for (i in 1:dim(dt)[2]){
    ...
    if (colnames(dt)[i]=="VehBody"){
      levels(dt$VehBody) <- c(levels(dt$VehBody), "autobus", "coupe",
                                     "autre microvan", "berline","SUV", "break",
                                     "camionnette")
      dt$VehBody[dt$VehBody == "bus"]<-"autobus"
      dt$VehBody[dt$VehBody == "coupe"]<-"coupe"
    ...
    if (colnames(dt)[i]=="VehUsage"){
      levels(dt$VehUsage) <- c(levels(dt$VehUsage), "privee", 
                                      "privee et trajet vers bureau", "professionnel", 
                                      "trajet professionnel" )
      dt$VehUsage[dt$VehUsage == "Private"]<-"privee"
      dt$VehUsage[dt$VehUsage == "Private+trip to office"]<-
      "privee et trajet vers bureau"
...
Pour ce data frame, l'âge moyen du conducteur est de 46,3 ans avec pour écart-type 14,9 ans. Le plus jeune conducteur a 18 ans(âge minimum légale pour conduire en France) et le plus âgé a 97 ans. L'écart interquartile (IQR), c'est-à-dire la mesure de dispersion qui s'obtient en faisant la différence entre le premier (25% des valeurs du data frame sont inférieures à ce quartile) et le troisième quartile(75 %), est de 23. Autrement dit, 50% des âges des conducteurs est compris entre 35 et 58 ans. Le coefficient de variation (CV), le rapport entre l'écart-type et la moyenne, est égale à 3. 
De même, en ce qui concerne l'usage du véhicule par son propriétaire, on remarquera que la plupart des personnes renseignées utilise leur véhicule pour les trajets privés et pour se rendre à leur bureau (44,1%).
...
Il n'est cependant pas judicieux d'appliquer une conversion numérique à ces colonnes puisqu'on leur attribue une valeur arbitraire nous faisant penser à une classification des différents facteurs possibles. Pour éviter cela, on va donc utiliser la méthode model.matrix() qui crée une matrice binaire spécifiant à quel facteur correspond une ligne du data frame.)
----------------
...
```{r, echo = TRUE}
freMPL2.pca <- PCA(freMPL2.active, ncp = 3, graph = FALSE)
```
...
Examinons donc ces valeurs propres afin de déterminer le nombre de composantes principales à prendre en considération. Les valeurs propres et la proportion de variances retenues par les composantes principales peuvent être extraites à l'aide de la fonction *get_eignevalue()* du package *factoextra*.  
...
```{r}
# création du data frame résumant nos résultats de calcul pour nos modèles GLM (fréquence et sévérité)

calcul_prime_glm <- data.frame(
  Id_police = 1:dim(freMPL2)[1],
  E_N = rep(0, times = dim(freMPL2)[1]),
  E_B = rep(0, times = dim(freMPL2)[1]),
  prime_pure = rep(0, times = dim(freMPL2)[1])
)

# Remplissage de la colonne E_N

# calcul pour un individu

calcul_E_N <- function(ligne){
  # Récupération des données nécessaires au calcul
  exposure <- ligne$Exposure
  driver_age <- ligne$DrivAge
  veh_age <- ligne$VehAge
  vehusage <- ligne$VehUsage
  bonusmalus <- ligne$BonusMalus
  veh_body <- ligne$VehBody
  veh_max_speed <- ligne$VehMaxSpeed
  # calcul
  e_n <- exposure*exp(as.numeric(glm1$coefficients["(Intercept)"]))
  e_n <- e_n*exp(coefficient_DrivAge[coefficient_DrivAge$DrivAge==driver_age,"coef"])
  e_n <- e_n*exp(coefficient_VehAge[coefficient_VehAge$VehAge==veh_age,"coef"])
  e_n <- e_n*exp(coefficient_VehUsage[coefficient_VehUsage$VehUsage==vehusage,"coef"])
  e_n <- e_n*exp(coefficient_BonusMalus[coefficient_BonusMalus$BonusMalus==bonusmalus,"coef"])
  e_n <- e_n*exp(coefficient_VehBody[coefficient_VehBody$VehBody==veh_body,"coef"])
  e_n <- e_n*exp(coefficient_VehMaxSpeed[coefficient_VehMaxSpeed$VehMaxSpeed==veh_max_speed,"coef"])
  return (e_n)
}

remplissage_tab_avec_E_N <- function(tableau){
  for (i in 1:dim(tableau)[1]){
    tableau$E_N[i] <- calcul_E_N(freMPL2[i,])
  }
  return (tableau)
}


calcul_prime_glm <- remplissage_tab_avec_E_N(calcul_prime_glm)
```
...
- Pour l'analyse des composantes prinicipales : 
