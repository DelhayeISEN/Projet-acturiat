...
plot(y_dpois1, type = "b", col = "blue", main =" lambda = 5", xlab = "x", ylab = "y", ylim = c(0,0.2))
plot(y_dpois2, type = "b", col = "red", main =" lambda = 10", xlab = "x", ylab = "y", ylim = c(0,0.2))
plot(y_dpois3, type = "b", col = "green", main =" lambda = 15", xlab = "x", ylab = "y", ylim = c(0,0.2))
```
...
glm1 <- glm(ClaimInd~DrivAge+HasKmLimit+offset(log(Exposure)), family=poisson, data=freMPL2)
summary(glm1)
glm1$aic
glm1$null.deviance
glm1$deviance
...
legend(x=10, y=0.5, legend = c("alpha = 1 et beta = 2", "alpha = 2 et beta = 2", "alpha = 3 et beta = 2","alpha = 5 et beta = 1", "alpha = 9 et beta = 0.5", "alpha = 7.5 et beta = 1", "alpha = 0.5 et beta = 1"),lty = 1, col = c("red", "orange", "yellow", "green", "black", "blue", "purple"))
```
...
legend(x=3, y=1, legend = c("mu = 1 et lambda = 1", "mu = 1 et lambda = 0.2", "mu = 1 et lambda = 3","mu = 3 et lambda = 1"),lty = 1, col = c("green", "red", "blue", "orange"))
```

```{r}
glm2 <- glm(ClaimAmount~DrivAge+VehAge+VehClass+VehBody+VehEnergy+offset(log(Exposure)), family=poisson("log"), data=freMPL2)
summary(glm2)
glm2$aic
glm2$null.deviance
glm2$deviance
...
recap2 <- data.frame(ClaimAmount = freMPL2$ClaimAmount,
                     Predict_ClaimAmount = round(predict2,0),
                     difference = freMPL2$ClaimAmount- round(predict2,0))
ratio2 <- round((dim(recap2[recap2$difference == 0,])[1]/dim(recap2)[1])*100,2)
ratio2
```
