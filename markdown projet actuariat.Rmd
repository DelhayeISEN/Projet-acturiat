...
levels(freMPL2$LicAge)
freMPL2 <- freMPL2[,c(-4)]
# Le ClaimAmount risque de fausser nos résultats pour la fréquence
freMPL2.freq <- freMPL2[,c(-19)]
```
...
knitr::kable(glmfreqtab[,c("term","estimate","p.value")])
```
...
dtsev <- data.frame(ClaimAmount = freMPL2$ClaimAmount,
                     Predict_ClaimAmount_final = round(predict(glmsevfinal, newdata = freMPL2, type = 'response'),2))
dtsev
```
...
CompDrivAge <- data.frame(
  DrivAge = levels(freMPL2.freq$DrivAge),
  sev_observee_somme = rep(0, times = 8),
  frequence_predite_init_model_somme = rep(0, times = 8),
  frequence_predite_final_model_somme = rep(0, times = 8)
)
