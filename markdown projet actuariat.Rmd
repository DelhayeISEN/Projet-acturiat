...
Test du modèle GLM avec BERNOUILLI : 
...
```{r, echo = TRUE}
glm1 <- glm(ClaimInd~., offset = log(Exposure), family=binomial(link="logit"), data=freMPL2)
```
...
head(recap1,10)
#recap1[recap1$difference<0.001,]
...
glm4 <- glm(ClaimAmount~., offset = ClaimInd, family=Gamma(link  = "log"), data=freMPL2.posclaim)
#summary(glm4)
predict4 <- predict(glm4, freMPL2.posclaim, type = 'response')
recap4 <- data.frame(ClaimAmount = freMPL2.posclaim$ClaimAmount,
                     Predict_ClaimAmount = predict4,
                     difference = abs(freMPL2.posclaim$ClaimAmount- predict4))
head(recap4,10)
```
