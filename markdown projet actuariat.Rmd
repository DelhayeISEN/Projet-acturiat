...
```{r, echo = TRUE}
freMPL2$DrivAge <- cut(freMPL2$DrivAge, c(17, 2:8*10, 100)) 

#calibration d’une loi de Poisson
glm1 <- glm(RiskVar~DrivAge+VehAge+VehClass+VehBody+VehEnergy, offset=log(Exposure), family=poisson("log"), data=freMPL2)
summary(glm1)
glm1$aic
glm1$null.deviance
glm1$deviance
...
recap1 <- data.frame(Riskvar = freMPL2$RiskVar,
                     Predict_RiskVar = round(predict1,0),
                     difference = freMPL2$RiskVar- round(predict1,0))
ratio1 <- dim(recap1[recap1$difference == 0,])[1]/dim(recap1)[1]
ratio1
```
