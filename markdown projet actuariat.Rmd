...
dt <- data.frame(
  ...
  frequence = c(test1$frequence_obs_moyenne,test1$frequence_pred_moyenne),
...
En <- data.frame(
  Id_police = seq(1:dim(freMPL2)[1]),
  E = rep(0, dim(freMPL2)[1])
)
...
```{r}
freMPL2[1,c("DrivAge", "VehAge", "VehUsage", "BonusMalus", "VehBody", "VehMaxSpeed", "ClaimAmount","Exposure")]
```
```{r}
summary(glm1)
```
...
for (i in 1:dim(freMPL2)[1]){
  ...
  testgam1[testgam1$DrivAge==age,"nb_individu"] <- testgam1[testgam1$DrivAge==age,"nb_individu"]+1
  testgam1[testgam1$DrivAge==age,"total_obs"] <-
    testgam1[testgam1$DrivAge==age,"total_obs"]+(freMPL2$ClaimInd[i]/freMPL2$Exposure[i])
  testgam1[testgam1$DrivAge==age,"total_pred"] <-
    testgam1[testgam1$DrivAge==age,"total_pred"]+(predictgam1[i]/freMPL2$Exposure[i])
}


for (j in 1:dim(testgam1)[1]){
  if (testgam1$nb_individu[j]!=0){
    testgam1[j,"frequence_obs"] <- testgam1[j,"total_obs"]/ testgam1[j,"nb_individu"]
    testgam1[j,"frequence_pred"] <- testgam1[j,"total_pred"]/ testgam1[j,"nb_individu"] 
  }
}

plot(testgam1$DrivAge,testgam1$frequence_obs, type = "b")
lines(testgam1$DrivAge,testgam1$frequence_pred, type = "b", col = "red")
```
