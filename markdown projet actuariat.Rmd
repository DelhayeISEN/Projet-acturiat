...
|    Le but de ce projet est de proposer un tariﬁcateur en se basant sur deux méthodes : les modèles linéaires généralisés (GLM) et les modèles additifs généralisés (GAM). Ces derniers sont une extension des GLM (proposée par McCullagh et Nelder, 1989) en considérant une approche non-paramétrique pour le prédicteur.  
...
- **`r colnames_freMPL2[16]`** : cela correspond au type d'énergie consommée par le véhicule
...
Cette fonction (appelée nettoyage_dataframe) prend l'un des deux data frames en paramètre et effectue les opérations suivantes :
...
|    On constate ainsi que, pour ce data frame, l'âge moyen du conducteur est de 46,3 ans avec pour écart-type 14,9 ans. Le plus jeune conducteur a 18 ans(âge minimum légal pour conduire en France) et le plus âgé a 97 ans. L'écart interquartile (IQR), c'est-à-dire la mesure de dispersion qui s'obtient en faisant la différence entre le premier (25% des valeurs du data frame sont inférieures à ce quartile) et le troisième quartile (75 %), est de 23. Autrement dit, 50% des conducteurs ont un âge compris entre 35 et 58 ans. Le coefficient de variation (CV), le rapport entre l'écart-type et la moyenne, est égal à 3.  
...
|    On remarquera donc que les variables DrivAge et LicAge sont bien représentées par nos axes principaux tandis que la variable RiskVar n'est pas bien représentée par nos axes. 
...
La ligne en pointillés rouge, sur les graphiques ci-dessus, indique la contribution moyenne attendue (dans notre cas, il est de 12,5%). Pour une composante donnée, une variable avec une contribution supérieure à ce seuil pourrait être considérée comme importante pour contribuer à la composante.
...
|    Au final, on notera que 5 de nos variables ont plus d'importance que les autres : l'age du conducteur, l'âge de la licence de ce conducteur, son bonus ou son malus, s'il a eu un accident pendant qu'il était assuré et le montant auquel il peut prétendre.  
...
$$B:\mathbb{R}\rightarrow\mathbb{R} \& \theta \in \iff \subset \mathbb{R}^k$$ est le vecteur de paramètres naturels.
...
|    Enfin la structure des erreurs va donc devoir être adaptée par rapport à nos modèles linéaires classiques afin qu’ils puissent correspondre à nos nouvelles données. Pour cela il existe plusieurs lois comme la loi de Poisson ou la loi Binomiale nous offrant une distribution des erreurs et des réponses qui seront différentes.
...
|    Ainsi, dans notre cas les distributions de Poisson ou binomiales négatives peuvent être utilisées pour représenter les fréquences des sinistres.
...
Bien que nous ne possédons pas la totalité de ces informations (par exemple, l'endroit où le véhicule est stationné ou le nombre de kilomètres parcourus), nous avons une quantité suffisante d'informations pour pouvoir calculer notre propre prime d'assurance.
...
|    Pour calculer l'espérance du nombre de sinistres, autrement la fréquence d'accident sur la période d'exposition, il suffit d'appliquer la formule suivante : 
$$E(N)=g^{-1} ( \sum a_i X_i )$$

Avec: 

- g^-1 : la fonction réciproque de la fonction de lien (dans notre modèle, la fonction de lien est logit(x) : $$log_{it}(x) = ln(\frac{x}{1-x})$$) et donc sa fonction réciproque est : $$sigmoid(x) = \frac{1}{1+e^{-x}}$$)
- a_i : le coefficient associé à un caractéristique de notre police. Pour rappel, le coefficient associé à l'offset est égal à 1).
- X_i : une caractéristique de notre police d'assurance. 

|    Fort heureusement pour nous, il existe une fonction R capable de calculer les fréquences selon les individus et selon les modèles GLM. Cette fonction est appelé **predict()** et s'implémente de la façon suivante : 
> predict(glmfreqfinal, freMPL2, type = 'response'),2)

|    Elle prend en paramètres un modèle linéaire (ici, glmfreqfinal), une base de données sur laquelle effectuée ces prédictions (freMPL2) et le type	de prediction (response ou autre). Dans notre cas, nous avons choisi le type 'response' qui nous renvoie l'espérance moyenne de la prédiction (donc ici notre fréquence). L'autre type nous aurait renvoyé la somme des coefficients (donc non transformée par la fonction réciproque). 
...
|    Ainsi, la prédiction faite pour les 3 premiers individus (dont les caractéristiques figurent au debut de ce rapport) donnent les résultats suivants : `r predind1`, `r predind2` et `r predind3`.
|    Selon notre modèle, le premier individu avait 5% de chances d'avoir un accident durant la période pour laquelle il était assuré.

|    Par curiosité, nous avons également décidé de voir les influences de l'âge du conducteur mais aussi de l'âge du véhicule sur cette fréquence : 

- Pour l'âge des conducteurs : 
...
CompDrivAge <- data.frame(
  ...
  nb_individu = rep(0, times = 8),
  frequence_predite_somme = rep(0, times = 8),
  frequence_predite_moyenne = rep(0, times = 8)
)

for (i in 1:dim(freMPL2[1])){
  age_conducteur <- freMPL2$DrivAge[i]
  CompDrivAge[CompDrivAge$DrivAge == age_conducteur,"nb_individu"] <- CompDrivAge[CompDrivAge$DrivAge == age_conducteur,"nb_individu"]+1
  CompDrivAge[CompDrivAge$DrivAge == age_conducteur,"frequence_predite_somme"] <- CompDrivAge[CompDrivAge$DrivAge == age_conducteur,"frequence_predite_somme"]+dtfreq$Predict_ClaimInd_final[i]
}

for(j in 1:length(levels(CompDrivAge$DrivAge))){
  CompDrivAge[j,"frequence_predite_moyenne"] <- round(CompDrivAge[j,"frequence_predite_somme"]/CompDrivAge[j,"nb_individu"],2)
}

knitr::kable(CompDrivAge[,c("DrivAge","frequence_predite_moyenne")])
```

- Pour l'âge des véhicules : 
...
CompVehAge <- data.frame(
  ...
  nb_individu = rep(0, times = 9),
  frequence_predite_somme = rep(0, times = 9),
  frequence_predite_moyenne = rep(0, times = 9)
)

for (i in 1:dim(freMPL2[1])){
  age_vehicule <- freMPL2$VehAge[i]
   CompVehAge[CompVehAge$VehAge == age_vehicule,"nb_individu"] <- CompVehAge[CompVehAge$VehAge == age_vehicule,"nb_individu"]+1
  CompVehAge[CompVehAge$VehAge == age_vehicule,"frequence_predite_somme"] <- CompVehAge[CompVehAge$VehAge == age_vehicule,"frequence_predite_somme"]+dtfreq$Predict_ClaimInd_final[i]
}

for(j in 1:length(levels(CompVehAge$VehAge))){
  CompVehAge[j,"frequence_predite_moyenne"] <- round(CompVehAge[j,"frequence_predite_somme"]/CompVehAge[j,"nb_individu"],2)
}

knitr::kable(CompVehAge[,c("VehAge","frequence_predite_moyenne")])
```

On constate donc que l'âge du conducteur n'influe pas énormément sur la fréquence de sinistre hormis chez les jeunes conducteurs (les apprentis) puisque leur fréquence est de 10% environ contre 5% environ pour les autres catégories d'âge. 
De même, l'âge du véhicule n'influence pas la fréquence de sinistres ce qui explique probablement le fait que cette variable n'ait pas été retenue pour établir notre modèle GLM.

### Sévérité des sinistres

#### Mise en place et exécution du modèle GLM

|    Après avoir déterminé la fréquence des sinistres, il nous faut maintenant calculer la sévérité de ces sinistres pour une police donnée.  
|    Pour cela, nous allons nous intéresser à la variable ClaimAmount qui correspond à un montant auquel peut prétendre l'assuré en cas d'accident. Si le montant est nul, c'est que la personne n'a pas eu d'accident sinon c'est qu'il en a eu au moins un. 
|    Pour modéliser la sévérité des sinistres, la loi la plus utilisée est la loi de probabilité Gamma. C'est donc cette loi que nous allons utiliser pour faire notre prédiction. Le résultat de la prédiction de notre modèle GLM correspondra à une valeur numérique traduisant la sévérité de l'accident : Plus la valeur sera élévée, plus l'accident sera grave.
```{r}
freMPL2 <- freMPL2save
```

|    Avant d'appliquer notre modèle, il est important de s'assurer que notre modèle d'entrainement ne contienne uniquement des valeurs non nulles. Pour cela, on utilise la commande *freMPL2[freMPL2$ClaimAmount != 0,]* qui conservera ces valeurs.

```{r}
freMPL2.posclaim <- freMPL2[freMPL2$ClaimAmount != 0,]
```

|    Nous pouvons maintenant passer à la modélisation de la sévérité des sinistres grâce à notre GLM.

|    Nous allons donc, comme pour la modélisation des sinistres, éxécuter un premier modèle GLM où la variable à expliquer est ClaimAmount, les variables explicatives correspondent à l'ensemble des autres variables de notre tableau. 
|    Pour le type de loi, nous utilisons donc la famille Gamma et la fonction de lien associée log qui est le logarithme népérien. 

|    Voici le premier modèle que nous avons utilisé dont le résultat figure en Annnexes au paragraphe "Modèle GLM pour sévérité n°=1" : 
```{r, echo = TRUE}
glmsevinit <- glm(ClaimAmount~., family=Gamma(link = "log"), data=freMPL2.posclaim)
```

|    Comme pour la fréquence, nous allons appliquer la fonction **step()** sur ce premier modèle afin d'obtenir un modèle plus proche de la vérité. Voici le modèle qui résulte de cette fonction :   
```{r}
#glmsevfinal<- step(glmsevinit)
```

```{r, echo = TRUE}
glmsevfinal <- glm(ClaimAmount ~ Exposure + LicAge + RecordBeg + VehAge +
                     MariStat + VehUsage + DrivAge + HasKmLimit + 
                     BonusMalus + VehBody + VehEngine + VehEnergy +
                     VehMaxSpeed + VehClass + RiskVar + Garage,
                   family=Gamma(link = "log"), data=freMPL2.posclaim)
```

|    On remarque que, comme pour la fréquence, le modèle final a conservé une grande majorité des variables comme l'âge du conducteur, le type de moteur du véhicule ou encore la variable associée au bonus et au malus et à retirer des variables comme l'âge du véhicule pour modéliser la fréquence.  
...
|    On remarquera que notre nouveau modèle n'a que très peu varié par rapport à l'ancien (augmentation de 42 AIC et baisse de 111 en déviance). Notre modèle s'est donc légèrement éloigné de la vérité (AIC) mais est plus linéaire puisque la déviance a diminuée de 111. 

#### Calcul de l'espérance de la sévérité des sinistres (E(B))

```{r}
freMPL2 <- freMPL2[freMPL2$VehEngine!="GPL",]
```

|    Pour calculer l'espérance de la sévérité des sinistres, il suffit d'appliquer la même formule que pour E(N) : 
$$E(B)=g^{-1}  \sum  a_i X_i $$

avec, cette fois-ci, g-1 : la fonction réciproque de la fonction de lien (dans notre modèle, la fonction de lien est log(x) : $$log(x) = ln(x)$$) et donc sa fonction réciproque est la fonction exponentielle.
...
predindsev1 <- dtsev[1,2]
predindsev2 <- dtsev[2,2]
predindsev3 <- dtsev[3,2]
```

|    Ainsi, la prédiction faite (grâce à la fonction **predict()**) pour les 3 premiers individus (dont les caractéristiques figurent au debut de ce rapport) donnent les résultats suivants : `r predindsev1`, `r predindsev2` et `r predindsev3`.
...
CompDrivAge1 <- data.frame(
  DrivAge = levels(freMPL2$DrivAge),
  nb_individu = rep(0, times = 8),
  montant_predit_somme = rep(0, times = 8),
  montant_predit_moyenne = rep(0, times = 8)
)

for (i in 1:dim(freMPL2[1])){
  age_conducteur <- freMPL2$DrivAge[i]
  CompDrivAge1[CompDrivAge1$DrivAge == age_conducteur,"nb_individu"] <- CompDrivAge1[CompDrivAge1$DrivAge == age_conducteur,"nb_individu"]+1
  CompDrivAge1[CompDrivAge1$DrivAge == age_conducteur,"montant_predit_somme"] <- CompDrivAge1[CompDrivAge1$DrivAge == age_conducteur,"montant_predit_somme"]+dtsev$Predict_ClaimAmount[i]
}

for(j in 1:length(levels(CompDrivAge1$DrivAge))){
  CompDrivAge1[j,"montant_predit_moyenne"] <- round(CompDrivAge1[j,"montant_predit_somme"]/CompDrivAge[j,"nb_individu"],2)
}

knitr::kable(CompDrivAge1[,c("DrivAge","montant_predit_moyenne")])
```
...
CompVehAge1 <- data.frame(
  VehAge = levels(freMPL2$VehAge),
  nb_individu = rep(0, times = 9),
  montant_predit_somme = rep(0, times = 9),
  montant_predit_moyenne = rep(0, times = 9)
)

for (i in 1:dim(freMPL2[1])){
  age_vehicule <- freMPL2$VehAge[i]
  CompVehAge1[CompVehAge1$VehAge == age_vehicule,"nb_individu"] <- CompVehAge1[CompVehAge1$VehAge == age_vehicule,"nb_individu"]+1
  CompVehAge1[CompVehAge1$VehAge == age_vehicule,"montant_predit_somme"] <- CompVehAge1[CompVehAge1$VehAge == age_vehicule,"montant_predit_somme"]+dtsev$Predict_ClaimAmount[i]
}

for(j in 1:length(levels(CompVehAge1$VehAge))){
  CompVehAge1[j,"montant_predit_moyenne"] <- round(CompVehAge1[j,"montant_predit_somme"]/CompVehAge1[j,"nb_individu"],2)
}

knitr::kable(CompVehAge1[,c("VehAge","montant_predit_moyenne")])
```
...
En admettant que la fréquence n'a pas d'influence sur la sévérité et que les montants des sinistres ont le même comportement aléatoire, on a : E(X) = E(N).E(B).

Ainsi, pour calculer la prime pure, il suffit de faire le produit ligne par ligne des 2 prédictions. 
...
primeind1 <- calcul_prime(freMPL2[1,])
primeind2 <- calcul_prime(freMPL2[2,])
primeind3 <- calcul_prime(freMPL2[3,])
```

Voici les résultats que l'on obtient en prime pure pour les trois premiers individus du tableau freMPL2 (celui sur lequel on a entraîné notre modèle):

|individu| **E(N)**  	| **E(B)** 	|**prime pure**|
|:-:|:-:|:-:|:-:|
|1|`r predind1`|`r predindsev1`|`r primeind1`€|
|2|`r predind2`|`r predindsev2`|`r primeind2`€|
|3|`r predind3`|`r predindsev3`|`r primeind3`€|
...
Nous pouvons également obtenir les primes pures de notre autre tableau (freMPL1). Voici les résultats pour les 3 premiers individus :

```{r}
predind4 <- round(predict(glmfreqfinal, freMPL1[1,], type = 'response'),2)
predind5 <- round(predict(glmfreqfinal, freMPL1[2,], type = 'response'),2)
predind6 <- round(predict(glmfreqfinal, freMPL1[3,], type = 'response'),2)
predindsev4 <- round(predict(glmsevfinal, freMPL1[1,], type = 'response'),2)
predindsev5 <- round(predict(glmsevfinal, freMPL1[2,], type = 'response'),2)
predindsev6 <- round(predict(glmsevfinal, freMPL1[3,], type = 'response'),2)
primeind4 <- calcul_prime(freMPL1[1,])
primeind5 <- calcul_prime(freMPL1[2,])
primeind6 <- calcul_prime(freMPL1[3,])
```

|individu| **E(N)**  	| **E(B)** 	|**prime pure**|
|:-:|:-:|:-:|:-:|
|1|`r predind4`|`r predindsev4`|`r primeind4`€|
|2|`r predind5`|`r predindsev5`|`r primeind5`€|
|3|`r predind6`|`r predindsev6`|`r primeind6`€|
...
|    La communication entre nous s’est faite via Messenger ou Discord, cela nous a permis d’échanger par messagerie instantanée et par vocal, afin également d’avoir un historique des différents éléments que nous avions partagés.

# Conclusion

|    Durant ce projet, nous avons rencontré plusieurs difficultés :

-	Adapter la partie théorique afin qu’elle soit compréhensible par un non-spécialiste de l’assurance, en particulier pour les GLMs.
-	Juger si la qualité des résultats des statistiques descriptives était satisfaisante, pour l’ACP et l’AFC.
-	Tirer des conclusions de ce que l’on a obtenu, des modélisations, faire des choix statistiques.
-	La modélisation de la sévérité des sinistres à l’aide des GLMs (polices en GPL). 
-	Juger des données que l’on peut exploiter ou non, et celles que l’on doit modifier.

|    Ce projet a été très enrichissant, il nous a permis de nous familiariser avec le monde de l’assurance. Nous avons dû faire des choix : évincer certaines données afin de ne garder que les principales, modifier des données et choisir certaines lois afin qu’elles soient adaptées au mieux aux différents problèmes.
|    Cependant, par manque de temps, la modélisation de la fréquence et de la sévérité des sinistres à l’aide des GAMs n’a pas été faite. Cela est donc une amélioration possible du projet.  
|    Nous avons cependant jugé util d'apporter des connaissances sur les modèles additifs généralisés en tant qu'ouverture pour une continuation du projet.

# Ouverture sur les GAM

|    Les Modèles Additifs Généralisés (GAM) sont des modèles visant à regrouper les propriétés des Modèles Linéaires Généralisés (GLM) ainsi que des modèles additifs.


|    Les modèles additifs proposent une regression non paramétrique de notre modèle et sont une généralisation de la régression multiple. Ainsi dans une regression linéaire, on calcule un ajustement linéaire par les moindres carrés pour un ensemble de variables X appelé prédicteurs, afin de prévoir une variable dépendante Y.
$$Y= b_0 + b_1*X_1+...+b_m*X_m$$
Ici, les X représentent les m valeurs des variables prédictives, les b rerprésentent les coefficients de régression et Y notre variable dépendante.


|    Les modèles additifs généralisés diffèrent des modèles linéaires généraux sur deux aspects :  
- La distribution de la variable dépendante n'est pas obligatoirement normale. Celle-ci peut être discontinue comme dans le cas d'une distribution binomiale.  
- La fonction de lien lie notre variable dépendante et nos variables prédictives par une fonction qui n'est pas forcément linéaire. La relation s'exprime ainsi :
$$Y=g(b_0+b_1*X_1+...+b_m*X_m)$$
où g représente une fonction.  

|    L'objectif des GAM est de maximiser la qualité de la prévision d'une variable dépendante Y à partir de diverses distributions en estimant des fonctions non-paramétriques des variables prédictives liées à la variable dépendante par la fonction de lien.

Ces distributions pour la variable dépendante peuvent suivre une distribution Normale, Gamma, Binomiale ou Poisson.

Ainsi nous pouvons écrire un modèle additif généralisé de cette forme :
$$g^{-1}(\mu Y)=\sum_i (f_i(X_i))$$
où g est une fonction, μY est la valeur théorique de Y et f notre fonction lien avec X les paramètres prédictifs.
...
- Pour les GAM :
<https://www.statsoft.fr/concepts-statistiques/modeles-additifs-generalises/modeles-additifs-generalises.php>
  
## Littérature

-Mathématiques de l'assurance non Vie T.1 et T.2 par A. Charpentier et M. Denuit  
-Generalized Linear Models. Chapman et Hall par T.J Hastie et R.J Tibshirani  
-R: A language and Environment for Statistical Computing par R Core Team  
-Cours sur l'Actuariat Non-Vie par C. Dutang  

# Annexes

## Affichage de l'implémentation de la fonction nettoyage_dataframe : 
...
### Modèle GLM pour sévérité n°= 1 :

```{r}
glmsevtab <- tidy(glmsevinit)
knitr::kable(glmsevtab[,c("term","estimate","p.value")])
```

### Modèle GLM pour sévérité n°= 2 : 
