...
```{r}
freMPL2$LicAge <- cut(freMPL2$LicAge, breaks = c(-1,36,1:8*120))
...
freMPL2.freq <- freMPL2[,c(-4,-19)]
```

```{r, results='hide', echo = TRUE}
glmfreqinit <- glm(ClaimInd~., offset = log(Exposure), family=binomial(link="logit"), data=freMPL2.freq)
#glmfreqfinal <- step(glmfreqinit)
glmfreqfinal <- glm(ClaimInd ~ Exposure + LicAge + RecordBeg + VehUsage + DrivAge + 
    BonusMalus + VehBody + VehEngine + VehClass + RiskVar, offset = log(Exposure), family=binomial(link="logit"), data=freMPL2.freq)
```
...
glmfreqfinal$aic
print("null.deviance")
glmfreqfinal$null.deviance
print("deviance")
glmfreqfinal$deviance
print("iter")
glmfreqfinal$iter
```
...
library(broom)
glmfreqtab <- tidy(glmfreqfinal)
knitr::kable(glmfreqtab[,c("term","estimate")])
```


```{r}
dtfreq <- data.frame(ClaimInd = freMPL2$ClaimInd,
                     Predict_ClaimInd_init = round(predict(glmfreqinit, freMPL2.freq, type = 'response'),2),
                     Predict_ClaimInd_final = round(predict(glmfreqfinal, freMPL2.freq, type = 'response'),2)
                     )
```
...
CompDrivAge <- data.frame(
  DrivAge = levels(freMPL2.freq$DrivAge),
  frequence_observee_somme = rep(0, times = 8),
  frequence_predite_init_model_somme = rep(0, times = 8),
...
for (i in 1:dim(freMPL2[1])){
  ...
  CompDrivAge[CompDrivAge$DrivAge == age_conducteur,"frequence_predite_init_model_somme"] <- CompDrivAge[CompDrivAge$DrivAge == age_conducteur,"frequence_predite_init_model_somme"]+dtfreq$Predict_ClaimInd_init[i]
...
CompVehAge <- data.frame(
  VehAge = levels(freMPL2.freq$VehAge),
  frequence_observee_somme = rep(0, times = 9),
  frequence_predite_init_model_somme = rep(0, times = 9),
...
for (i in 1:dim(freMPL2[1])){
  ...
  CompVehAge[CompVehAge$VehAge == age_vehicule,"frequence_predite_init_model_somme"] <- CompVehAge[CompVehAge$VehAge == age_vehicule,"frequence_predite_init_model_somme"]+dtfreq$Predict_ClaimInd_init[i]
...
```{r, results='hide', echo = TRUE}
glmsevinit <- glm(ClaimAmount~.,offset = log(Exposure), family=Gamma(link = "log"), data=freMPL2.posclaim)
#glmsevfinal<- step(glmsevinit)
glmsevfinal <- glm(ClaimAmount~Exposure + RecordBeg + RecordEnd + VehAge + MariStat + 
    VehUsage + DrivAge + HasKmLimit + BonusMalus + VehBody + 
    VehEngine + VehEnergy + RiskVar + Garage,offset = log(Exposure), family=Gamma(link = "log"), data=freMPL2.posclaim)
```
...
glmsevfinal$aic
print("null.deviance")
glmsevfinal$null.deviance
print("deviance")
glmsevfinal$deviance
print("iter")
glmsevfinal$iter
```
...
knitr::kable(glmsevtab[,c("term","estimate")])
```
...
```{r}
# Pour ceux qui ont eu un accident


calcul_prime <- data.frame(
  identifiant_individu = 1:dim(freMPL2.posclaim)[1], 
  prediction_moyenne_frequence = round(predict(glmfreqfinal, freMPL2.posclaim, type = 'response'),2),
  prediction_moyenne_severite = round(predict(glmsevfinal, freMPL2.posclaim, type = 'response'),2),
  prime_pure = rep(0, times   = dim(freMPL2.posclaim)[1])
)



for (i in 1:dim(freMPL2.posclaim)[1]){
  calcul_prime$prime_pure[i] <- calcul_prime$prediction_moyenne_frequence[i]*calcul_prime$prediction_moyenne_severite[i]
}
...
fonction de lien logit : logit(x) = ln(X/(1-x))
fonction réciproque : sigmoid(x) = 1/(1+exp(-x))
