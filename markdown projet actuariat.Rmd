...
freMPL2$BonusMalus <- cut(freMPL2$BonusMalus, breaks = c(49,100,351))
...
FORMULE  : E(N/x) = g^(-1) (aX_i+e_i ) avec g la fonction de lien et e l'offset

Ainsi, pour une loi de Poisson, on a : E(N/x) = exp(aX_i+ln(Exposure))
                                              = Exposure*exp(aX_i)
                                              
```{r}
# Récupération des différents coefficients

# Pour DrivAge 

coefficient_DrivAge <- data.frame(
  DrivAge = levels(freMPL2$DrivAge),
  coef = rep(0, length(levels(freMPL2$DrivAge)))
  )

for (i in 1:length(levels(freMPL2$DrivAge))){
  newcoef <- as.numeric(glm1$coefficients[paste("DrivAge",levels(freMPL2$DrivAge)[i], sep = "")])
  if (!is.na(newcoef)){
    coefficient_DrivAge$coef[i] <- newcoef
  }
}

# Pour VehAge : 

coefficient_VehAge <- data.frame(
  VehAge = levels(freMPL2$VehAge),
  coef = rep(0, length(levels(freMPL2$VehAge)))
  )

for (i in 1:length(levels(freMPL2$VehAge))){
  newcoef <- as.numeric(glm1$coefficients[paste("VehAge",levels(freMPL2$VehAge)[i], sep = "")])
  if (!is.na(newcoef)){
    coefficient_VehAge$coef[i] <- newcoef
  }
}

# Pour VehUsage : 

coefficient_VehUsage <- data.frame(
  VehUsage = levels(freMPL2$VehUsage),
  coef = rep(0, length(levels(freMPL2$VehUsage)))
  )

for (i in 1:length(levels(freMPL2$VehUsage))){
  newcoef <- as.numeric(glm1$coefficients[paste("VehUsage",levels(freMPL2$VehUsage)[i], sep = "")])
  if (!is.na(newcoef)){
    coefficient_VehUsage$coef[i] <- newcoef
  }
}

# Pour BonusMalus : 

coefficient_BonusMalus <- data.frame(
  BonusMalus = levels(freMPL2$BonusMalus),
  coef = rep(0, length(levels(freMPL2$BonusMalus)))
  )

for (i in 1:length(levels(freMPL2$BonusMalus))){
  newcoef <- as.numeric(glm1$coefficients[paste("BonusMalus",levels(freMPL2$BonusMalus)[i], sep = "")])
  if (!is.na(newcoef)){
    coefficient_BonusMalus$coef[i] <- newcoef
  }
}

# Pour VehBody : 

coefficient_VehBody <- data.frame(
  VehBody = levels(freMPL2$VehBody),
  coef = rep(0, length(levels(freMPL2$VehBody)))
  )

for (i in 1:length(levels(freMPL2$VehBody))){
  newcoef <- as.numeric(glm1$coefficients[paste("VehBody",levels(freMPL2$VehBody)[i], sep = "")])
  if (!is.na(newcoef)){
    coefficient_VehBody$coef[i] <- newcoef
  }
}

# Pour VehMaxSpeed : 

coefficient_VehMaxSpeed <- data.frame(
  VehMaxSpeed = levels(freMPL2$VehMaxSpeed),
  coef = rep(0, length(levels(freMPL2$VehMaxSpeed)))
  )

for (i in 1:length(levels(freMPL2$VehMaxSpeed))){
  newcoef <- as.numeric(glm1$coefficients[paste("VehMaxSpeed",levels(freMPL2$VehMaxSpeed)[i], sep = "")])
  if (!is.na(newcoef)){
    coefficient_VehMaxSpeed$coef[i] <- newcoef
  }
}
```

```{r}
# Calcul des différentes fréquences prédites

# Pour DrivAge

frequence_predite_DrivAge <- data.frame(
  DrivAge = levels(freMPL2$DrivAge),
  frequence_predite = rep(exp(as.numeric(glm1$coefficients["(Intercept)"])), length(levels(freMPL2$DrivAge)))
  )

for (i in 1:length(levels(freMPL2$DrivAge))){
  frequence_predite_DrivAge$frequence_predite[i] <- frequence_predite_DrivAge$frequence_predite[i]*exp(coefficient_DrivAge$coef[i])  
}

# Pour VehAge

frequence_predite_VehAge <- data.frame(
  VehAge = levels(freMPL2$VehAge),
  frequence_predite = rep(exp(as.numeric(glm1$coefficients["(Intercept)"])), length(levels(freMPL2$VehAge)))
  )

for (i in 1:length(levels(freMPL2$VehAge))){
  frequence_predite_VehAge$frequence_predite[i] <- frequence_predite_VehAge$frequence_predite[i]*exp(coefficient_VehAge$coef[i])  
}

# Pour VehUsage

frequence_predite_VehUsage <- data.frame(
  VehUsage = levels(freMPL2$VehUsage),
  frequence_predite = rep(exp(as.numeric(glm1$coefficients["(Intercept)"])), length(levels(freMPL2$VehUsage)))
  )

for (i in 1:length(levels(freMPL2$VehUsage))){
  frequence_predite_VehUsage$frequence_predite[i] <- frequence_predite_VehUsage$frequence_predite[i]*exp(coefficient_VehUsage$coef[i])  
}

# Pour BonusMalus

frequence_predite_BonusMalus <- data.frame(
  BonusMalus = levels(freMPL2$BonusMalus),
  frequence_predite = rep(exp(as.numeric(glm1$coefficients["(Intercept)"])), length(levels(freMPL2$BonusMalus)))
  )

for (i in 1:length(levels(freMPL2$BonusMalus))){
  frequence_predite_BonusMalus$frequence_predite[i] <- frequence_predite_BonusMalus$frequence_predite[i]*exp(coefficient_BonusMalus$coef[i])  
}

# Pour VehBody

frequence_predite_VehBody <- data.frame(
  VehBody = levels(freMPL2$VehBody),
  frequence_predite = rep(exp(as.numeric(glm1$coefficients["(Intercept)"])), length(levels(freMPL2$VehBody)))
  )

for (i in 1:length(levels(freMPL2$VehBody))){
  frequence_predite_VehBody$frequence_predite[i] <- frequence_predite_VehBody$frequence_predite[i]*exp(coefficient_VehBody$coef[i])  
}

# Pour VehMaxSpeed

frequence_predite_VehMaxSpeed <- data.frame(
  VehMaxSpeed = levels(freMPL2$VehMaxSpeed),
  frequence_predite = rep(exp(as.numeric(glm1$coefficients["(Intercept)"])), length(levels(freMPL2$VehMaxSpeed)))
  )

for (i in 1:length(levels(freMPL2$VehMaxSpeed))){
  frequence_predite_VehMaxSpeed$frequence_predite[i] <- frequence_predite_VehMaxSpeed$frequence_predite[i]*exp(coefficient_VehMaxSpeed$coef[i])  
}
```

```{r}
# Comparaison résultat et observation

# Pour DrivAge

test1 <- data.frame(
  DrivAge = levels(freMPL2$DrivAge),
  nb_individu = rep(0,times = length(levels(freMPL2$DrivAge))),
  total_obs = rep(0,times = length(levels(freMPL2$DrivAge))),
  total_pred = rep(0,times = length(levels(freMPL2$DrivAge))),
  frequence_obs_moyenne = rep(0,times = length(levels(freMPL2$DrivAge))),
  frequence_pred_moyenne = rep(0,times = length(levels(freMPL2$DrivAge))),
  frequence_predite_par_groupe = frequence_predite_DrivAge$frequence_predite
)

for (i in 1:dim(freMPL2)[1]){
  groupe_age <- freMPL2$DrivAge[i]
  test1[test1$DrivAge==groupe_age,"nb_individu"] <- test1[test1$DrivAge==groupe_age,"nb_individu"]+1
  test1[test1$DrivAge==groupe_age,"total_obs"] <-
    test1[test1$DrivAge==groupe_age,"total_obs"]+(freMPL2$ClaimInd[i]/freMPL2$Exposure[i])
  test1[test1$DrivAge==groupe_age,"total_pred"] <-
    test1[test1$DrivAge==groupe_age,"total_pred"]+(predict1[i]/freMPL2$Exposure[i])
}

for (j in 1:dim(test1)[1]){
  if (test1$nb_individu[j]!=0){
    test1[j,"frequence_obs_moyenne"] <- test1[j,"total_obs"]/ test1[j,"nb_individu"]
    test1[j,"frequence_pred_moyenne"] <- test1[j,"total_pred"]/ test1[j,"nb_individu"] 
  }
}
...
test1_1 <- data.frame(
  VehAge = levels(freMPL2$VehAge),
  nb_individu = rep(0,times = length(levels(freMPL2$VehAge))),
  total_obs = rep(0,times = length(levels(freMPL2$VehAge))),
  total_pred = rep(0,times = length(levels(freMPL2$VehAge))),
  frequence_obs_moyenne = rep(0,times = length(levels(freMPL2$VehAge))),
  frequence_pred_moyenne = rep(0,times = length(levels(freMPL2$VehAge))),
  frequence_predite_par_groupe = frequence_predite_VehAge$frequence_predite
)

for (i in 1:dim(freMPL2)[1]){
  groupe_age <- freMPL2$VehAge[i]
  test1_1[test1_1$VehAge==groupe_age,"nb_individu"] <- test1_1[test1_1$VehAge==groupe_age,"nb_individu"]+1
  test1_1[test1_1$VehAge==groupe_age,"total_obs"] <-
    test1_1[test1_1$VehAge==groupe_age,"total_obs"]+(freMPL2$ClaimInd[i]/freMPL2$Exposure[i])
  test1_1[test1_1$VehAge==groupe_age,"total_pred"] <-
    test1_1[test1_1$VehAge==groupe_age,"total_pred"]+(predict1[i]/freMPL2$Exposure[i])
}

for (j in 1:dim(test1_1)[1]){
  if (test1_1$nb_individu[j]!=0){
    test1_1[j,"frequence_obs_moyenne"] <- test1_1[j,"total_obs"]/ test1_1[j,"nb_individu"]
    test1_1[j,"frequence_pred_moyenne"] <- test1_1[j,"total_pred"]/ test1_1[j,"nb_individu"] 
  }
}
...
#########################################
dt1 <- data.frame(
  DrivAge = c(levels(freMPL2$DrivAge),levels(freMPL2$DrivAge),levels(freMPL2$DrivAge)),
  frequence = c(test1$frequence_obs_moyenne,test1$frequence_pred_moyenne,test1$frequence_predite_par_groupe),
  type = c(rep("frequence_obs_moyenne",times = length(levels(freMPL2$DrivAge))),rep("frequence_pred_moyenne",times = length(levels(freMPL2$DrivAge))),rep("frequence_predite_par_groupe",times = length(levels(freMPL2$DrivAge))))
)

dt1_1 <- data.frame(
  VehAge = c(levels(freMPL2$VehAge),levels(freMPL2$VehAge),levels(freMPL2$VehAge)),
  frequence = c(test1_1$frequence_obs_moyenne,test1_1$frequence_pred_moyenne,test1_1$frequence_predite_par_groupe),
  type = c(rep("frequence_obs_moyenne",times = length(levels(freMPL2$VehAge))),rep("frequence_pred_moyenne",times = length(levels(freMPL2$VehAge))),rep("frequence_predite_par_groupe",times = length(levels(freMPL2$VehAge))))
)

ggplot(dt1) +
  ggtitle("Nombre de sinistres et leur prédiction selon l’âge du conducteur
")+
  geom_point(aes(x = DrivAge, y = frequence, color = type))+
  theme(legend.position="bottom")+
  theme(legend.title = element_blank())

ggplot(dt1_1) +
  ggtitle("Nombre de sinistres et leur prédiction selon l’âge du véhicule
")+
  geom_point(aes(x = VehAge, y = frequence, color = type))+
  theme(legend.position="bottom")+
  theme(legend.title = element_blank())
```
...
glm2 <- glm(ClaimInd~DrivAge+VehAge+VehUsage+BonusMalus+VehBody+VehMaxSpeed, offset = log(Exposure), family=quasipoisson("log"), data=freMPL2)
summary(glm2)
#glm2$aic
#glm2$null.deviance
#glm2$deviance
...
glm3 <- glm(ClaimInd~DrivAge+VehAge+VehUsage+BonusMalus+VehBody+VehMaxSpeed, offset = log(Exposure), family=negative.binomial(phi), data=freMPL2)
summary(glm3)
...
head(recap3,10)
recap3[recap3$difference<0.5 & recap3$ClaimInd == 1,]
 # Ce modèle ne fonctionne pas pour prédire les ClaimInd égaux à 1
```
...
```{r}
gam1 <- gam(ClaimInd~DrivAge+VehAge+VehUsage+BonusMalus+VehBody+VehMaxSpeed+ClaimAmount+offset(log(Exposure)), family=poisson("log"), data=freMPL2)
summary(gam1)
predictgam1 <- predict(gam1, freMPL2, type = 'response')
recapgam1 <- data.frame(ClaimInd = freMPL2$ClaimInd,
                     Predict_ClaimInd = predictgam1,
                     difference = abs(freMPL2$ClaimInd- predictgam1))
head(recapgam1,10)
recapgam1[recapgam1$difference<0.5 & recapgam1$ClaimInd == 1,]
 # Ce modèle ne fonctionne pas pour prédire les ClaimInd égaux à 1
...
test2 <- data.frame(
  ...
  frequence_obs_moyenne = rep(0,times = length(levels(freMPL2$DrivAge))),
  frequence_pred_moyenne = rep(0,times = length(levels(freMPL2$DrivAge))),
  frequence_predite_par_groupe = rep(0,times = length(levels(freMPL2$DrivAge))) 
)

for (i in 1:dim(freMPL2)[1]){
  groupe_age <- freMPL2$DrivAge[i]
  test2[test2$DrivAge==groupe_age,"nb_individu"] <- test2[test2$DrivAge==groupe_age,"nb_individu"]+1
  test2[test2$DrivAge==groupe_age,"total_obs"] <-
    test2[test2$DrivAge==groupe_age,"total_obs"]+(freMPL2$ClaimInd[i]/freMPL2$Exposure[i])
  test2[test2$DrivAge==groupe_age,"total_pred"] <-
    test2[test2$DrivAge==groupe_age,"total_pred"]+(predict1[i]/freMPL2$Exposure[i])
}

for (j in 1:dim(test2)[1]){
  if (test2$nb_individu[j]!=0){
    test2[j,"frequence_obs_moyenne"] <- test2[j,"total_obs"]/ test2[j,"nb_individu"]
    test2[j,"frequence_pred_moyenne"] <- test2[j,"total_pred"]/ test2[j,"nb_individu"] 
  }
}

dt2 <- data.frame(
  DrivAge = c(levels(freMPL2$DrivAge),levels(freMPL2$DrivAge)),
  frequence = c(test2$frequence_obs_moyenne,test2$frequence_pred_moyenne),
  type = c(rep("frequence_obs_moyenne",times = length(levels(freMPL2$DrivAge))),rep("frequence_pred_moyenne",times = length(levels(freMPL2$DrivAge))))
)
...
```{r}
gam2 <- gam(ClaimInd~DrivAge+VehAge+VehUsage+BonusMalus+VehBody+VehMaxSpeed+ClaimAmount+offset(log(Exposure)), family=quasipoisson("log"), data=freMPL2)
summary(gam2)
predictgam2 <- predict(gam2, freMPL2, type = 'response')
recapgam2 <- data.frame(ClaimInd = freMPL2$ClaimInd,
                     Predict_ClaimInd = predictgam2,
                     difference = abs(freMPL2$ClaimInd- predictgam2))
head(recapgam2,10)
recapgam2[recapgam2$difference<0.5 & recapgam2$ClaimInd == 1,]
 # Ce modèle ne fonctionne pas pour prédire les ClaimInd égaux à 1
...
for (i in 1:dim(freMPL2)[1]){
  ...
  testgam2[testgam2$DrivAge==age,"nb_individu"] <- testgam2[testgam2$DrivAge==age,"nb_individu"]+1
  testgam2[testgam2$DrivAge==age,"total_obs"] <-
    testgam2[testgam2$DrivAge==age,"total_obs"]+(freMPL2$ClaimInd[i]/freMPL2$Exposure[i])
  testgam2[testgam2$DrivAge==age,"total_pred"] <-
    testgam2[testgam2$DrivAge==age,"total_pred"]+(predictgam2[i]/freMPL2$Exposure[i])
}


for (j in 1:dim(testgam2)[1]){
  if (testgam2$nb_individu[j]!=0){
    testgam2[j,"frequence_obs"] <- testgam2[j,"total_obs"]/ testgam2[j,"nb_individu"]
    testgam2[j,"frequence_pred"] <- testgam2[j,"total_pred"]/ testgam2[j,"nb_individu"] 
  }
}

plot(testgam2$DrivAge,testgam2$frequence_obs, type = "b")
lines(testgam2$DrivAge,testgam2$frequence_pred, type = "b", col = "red")
```
...
gam3 <- gam(ClaimInd~DrivAge+VehAge+VehUsage+BonusMalus+VehBody+VehMaxSpeed+ClaimAmount+offset(log(Exposure)), family=negative.binomial(phi), data=freMPL2)
summary(gam3)
predictgam3 <- predict(gam3, freMPL2, type = 'response')
recapgam3 <- data.frame(ClaimInd = freMPL2$ClaimInd,
                     Predict_ClaimInd = predictgam3,
                     difference = abs(freMPL2$ClaimInd- predictgam3))
head(recapgam3,10)
recapgam3[recapgam3$difference<0.1 & recapgam3$ClaimInd == 1,]
 # Ce modèle ne fonctionne pas pour prédire les ClaimInd égaux à 1
...
for (i in 1:dim(freMPL2)[1]){
  age <- freMPL2$DrivAge[i]
  testgam3[testgam3$DrivAge==age,"nb_individu"] <- testgam3[testgam3$DrivAge==age,"nb_individu"]+1
  testgam3[testgam3$DrivAge==age,"total_obs"] <-
    testgam3[testgam3$DrivAge==age,"total_obs"]+(freMPL2$ClaimInd[i]/freMPL2$Exposure[i])
  testgam3[testgam3$DrivAge==age,"total_pred"] <-
    testgam3[testgam3$DrivAge==age,"total_pred"]+(predictgam3[i]/freMPL2$Exposure[i])
}


for (j in 1:dim(testgam3)[1]){
  if (testgam3$nb_individu[j]!=0){
    testgam3[j,"frequence_obs"] <- testgam3[j,"total_obs"]/ testgam3[j,"nb_individu"]
    testgam3[j,"frequence_pred"] <- testgam3[j,"total_pred"]/ testgam3[j,"nb_individu"] 
  }
}

plot(testgam3$DrivAge,testgam3$frequence_obs, type = "b")
lines(testgam3$DrivAge,testgam3$frequence_pred, type = "b", col = "red")
```
